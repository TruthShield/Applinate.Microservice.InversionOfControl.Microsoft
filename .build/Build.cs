...
{
    static string Version = new Version(0, 3, 0).ToString();
    ...
    Target Test => _ => _
        .DependsOn(Compile)
        //.Triggers(PublishToGithub) // UNDONE: FIX CI/CD
        .Executes(() =>
        {
    ...
    Target Pack => _ => _
      ...
      {
          DotNetPack(s => s
            .SetProject(Solution.GetProject("Applinate.Microservice.InversionOfControl.Microsoft"))
            .SetConfiguration(Configuration)
            .EnableNoBuild()
            .EnableNoRestore()
            .SetVersion(Version)
